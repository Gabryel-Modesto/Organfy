...
function normalizePaymentMethodName(name){
    ...
    if(normalizedName.length < 3) {
        throw new Error ("O nome da forma de pagamento deve ter ao menos, 3 caracteres!")
    };

    if(normalizedName.length > 100){
        throw new Error("O nome da forma de pagamento deve ter no máximo 100 caracteres!")
    };
...
async function create(paymentMethodData, userId) {
    ...
    const normalizedName =normalizePaymentMethodName(
        name_payment_method
    );
    ...
    if(existingPaymentMethod) {
        ...
        if(!existingPaymentMethod.active_payment_method) {
...
async function findById(idPaymentMethod, userId) {
    return await paymentMethodRepository.findById(
        idPaymentMethod,
        userId
    );
};
...
async function update(idPaymentMethod,userId,paymentMethodData) {
    ...
    return await paymentMethodRepository.update(idPaymentMethod,userId,dataToUpdate);
};
...
async function remove(idPaymentMethod, userId) {
    return await paymentMethodRepository.remove(
        idPaymentMethod,
        userId
    );
;}
